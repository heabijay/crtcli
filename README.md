...
Here you can use options from [app pkg install](#app-pkg-install) command like --restart, --force, ...
...
- `--output-filename | -n <FILENAME>` — Filename of the output package archive file.
    ...
    Defaults: Autogenerated — `PackageName_YYYY-MM-DD_HH-mm-ss.zip` for zip format and `PackageName.gz` for gzip. If there are multiple package folders, then the filename will be `Packages_YYYY-MM-DD_HH-mm-ss.zip`.
