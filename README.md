> [!NOTE]
> This branch is a proof of concept about generating schemas code (Autogenerated/Src) without an active Creatio instance. You only need the binary files of the .NET (Kestrel) based Creatio instance to generate schemas. By this feature, you will be able to regenerate schemas code locally, even without Creatio. **This feature** is experimental and **set to be held**, due to Creatio > ~8.1.1 starting to regenerate and compile packages on installation by itself, so you may not need to generate an Autogenerated folder.
