...
internal static class SysSettingsWithValue
{
    public record Request(bool IsCacheable) : IDbCommandMockRequest;
    ...
    public class Parser : IDbCommandMockRequestParser<Request>
    {
        public bool TryParse(IDbCommand dbCommand, out Request request)
        {
            ...
            var isCacheableParameter = (DbParameterMock)dbCommand.Parameters["P1"];
            var isCacheable = (bool)isCacheableParameter.Value!;

            request = new Request(isCacheable);
            return true;
        }
    }
    ...
    public class Handler(ILogger<Handler> logger) : IDbCommandMockRequestHandler<Request>
    {
        ...
        {
            logger.LogDebug("All SysSettingsWithValue query currently returning no actual values");
